...
import { WorkedWithCompaniesManager } from './WorkedWithCompaniesManager';
...
export const AdminDashboardModal: React.FC<AdminDashboardModalProps> = ({ isOpen, onClose }) => {
  ...
  // Fetch enquiries from secured API
  const fetchSubmissions = async (silent = false) => {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        if (!silent) {
          setError('Authentication session expired or invalid. Please log in as Admin.');
        }
        return;
      }
      ...
      if (res.status >= 500) {
        if (!silent) {
          setError('Server storage or database error (HTTP 500). Please try again.');
        }
        return;
      }
      ...
      if (!silent) {
        setError('Unable to load submissions from server. Please check your network connection.');
      }
    } finally {
      if (!silent) {
        setLoading(false);
  ...
  // Re-fetch when modal opens or filters change, and run 30s auto-refresh polling
  useEffect(() => {
    if (isOpen && isAdmin) {
  ...
  }, [isOpen, isAdmin, typeFilter, dateFilter, sortOrder]);
  ...
  const handleDeleteSubmission = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.error || 'Failed to delete submission record.');
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/85 backdrop-blur-md animate-fadeIn overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-7xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden relative text-left">
        
        {/* Modal Header */}
        <div className="flex flex-wrap items-center justify-between gap-4 p-5 sm:px-8 border-b border-slate-800 bg-slate-950/70">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg sm:text-xl font-black font-mono uppercase text-white tracking-wide">
              ...
              <p className="text-xs text-slate-400 font-mono">
                SR GROUP Website Form Submissions & Inquiries Log
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {isAdmin && (
              <span className="text-xs text-slate-300 bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-800 font-mono hidden sm:inline-block">
          ...
          <div className="p-12 text-center space-y-5 my-auto">
            ...
            <div className="space-y-2 max-w-md mx-auto">
              ...
              <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">
                This dashboard contains confidential client inquiries, quote requests, and candidate applications. You must authenticate with valid Administrator credentials to access this records database.
              </p>
            </div>
            <div>
              <button
                onClick={() => openAuthModal()}
                className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-slate-950 font-black text-xs px-6 py-3 rounded-xl uppercase tracking-wider shadow-lg inline-flex items-center gap-2 transition-transform hover:scale-102"
              >
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3 border-b border-slate-800 pb-4">
              <button
                ...
                className={`px-4 py-2.5 rounded-xl font-mono text-xs font-black uppercase tracking-wider flex items-center gap-2 transition-all shadow-md ${
            ...
            {adminActiveTab === 'companies' ? (
              ...
              <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-1">
                ...
                <p className="text-2xl font-black font-mono text-white">{metrics.total}</p>
              ...
              <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-1">
                ...
                <p className="text-2xl font-black font-mono text-blue-400">{metrics.contacts}</p>
              ...
              <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-1">
                ...
                <p className="text-2xl font-black font-mono text-amber-400">{metrics.quotes}</p>
              ...
              <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-1">
                ...
                <p className="text-2xl font-black font-mono text-purple-400">{metrics.careers}</p>
            ...
            {error && (
              <div className="bg-rose-500/10 border border-rose-500/30 p-3.5 rounded-xl flex items-center gap-3 text-xs text-rose-400">
                <AlertTriangle className="w-4 h-4 shrink-0" />
                <span>{error}</span>
              </div>
            )}

            {/* Controls Bar: Search, Filters, Sorting & CSV Export */}
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                ...
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder="Search by name, email, phone, company, service or keywords..."
                    className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-amber-400 pl-9 font-mono"
                  />
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="flex items-center gap-1.5 bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-700 px-3.5 py-2 rounded-lg text-xs font-bold font-mono transition-colors disabled:opacity-50"
                    title="Refresh List"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-slate-950 px-4 py-2 rounded-lg text-xs font-black uppercase tracking-wider transition-all shadow-md"
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {(['all', 'contact', 'quote', 'career'] as const).map((t) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[11px] font-mono uppercase font-bold transition-colors ${
                        typeFilter === t
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <div className="flex items-center gap-1">
                    ...
                    <select
                      ...
                      className="bg-slate-900 border border-slate-700 rounded-lg px-2.5 py-1 text-xs text-white focus:outline-none focus:border-amber-400 font-mono"
                    >
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-xl overflow-hidden shadow-xl">
              {loading ? (
                ...
                <div className="py-16 text-center text-slate-400 space-y-2 font-mono">
                  <FileText className="w-10 h-10 text-slate-600 mx-auto" />
                  <p className="text-sm font-bold text-slate-300">No submissions found.</p>
                  <p className="text-xs text-slate-500">
                    {searchQuery ? 'Try clearing your search query or adjusting filter parameters.' : 'No form submissions have been registered yet.'}
                  </p>
                </div>
              ) : (
                            ...
                            <td className="py-3 px-4 text-right whitespace-nowrap">
                              <div className="flex items-center justify-end gap-1.5">
                                ...
                                <button
                                  ...
                                  className="p-1.5 bg-rose-500/10 hover:bg-rose-500 text-rose-400 hover:text-white rounded border border-rose-500/20 transition-colors"
        ...
        {/* Footer info */}
        <div className="p-4 sm:px-8 border-t border-slate-800 bg-slate-950/90 flex flex-wrap items-center justify-between text-xs text-slate-500 font-mono gap-2">
          <span>Protected with HTTP-only Cookies & JWT Authorization</span>
          <span>Showing {filteredSubmissions.length} of {submissions.length} loaded records</span>
        </div>

      </div>

      {/* ------------------------------------------------------------- */}
      {/* SUBMISSION FULL DETAIL MODAL */}
      {/* ------------------------------------------------------------- */}
      {selectedSubmission && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-mono">
              ...
              <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1">
                ...
                <a
                  ...
                  className="text-amber-400 hover:underline block break-all font-bold"
                >
              ...
              <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1">
                ...
                <a
                  ...
                  className="text-amber-400 hover:underline block font-bold"
                >
